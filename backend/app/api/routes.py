...
from app.models import Search,Offer,Recommendation,SearchRun
...
def results(search_id:int,db:Session=Depends(get_db)):
    offers=db.scalars(select(Offer).where(Offer.search_id==search_id).order_by(Offer.best_price).limit(100)).all(); out=[]
    for o in offers:
        rec=db.scalar(select(Recommendation).where(Recommendation.offer_id==o.id).order_by(desc(Recommendation.created_at)).limit(1)); out.append({"id":o.id,"origin":o.origin,"destination":o.destination,"departure_date":o.departure_date,"return_date":o.return_date,"price":float(o.best_price),"currency":o.currency,"airlines":o.airlines,"stops":o.stops,"duration_minutes":o.duration_minutes,"providers":o.provider_count,"consensus":o.consensus_score,"price_level":o.price_level,"booking_url":o.booking_url,"recommendation":None if not rec else {"state":rec.state,"score":rec.score,"confidence":rec.confidence,"reasons":rec.reasons,"percentile":rec.historical_percentile}})
    return out
