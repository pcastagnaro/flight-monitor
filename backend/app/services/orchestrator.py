...
def providers():
    ps=[FlightPowersProvider(),SerpApiProvider(),TravelpayoutsProvider()]; enabled=[p for p in ps if p.enabled]; return enabled if enabled else [MockProvider()]
async def _safe(p,q):
    try: return p.name,await p.search(q),None
    except Exception as e: return p.name,[],f"{type(e).__name__}: {e}"
async def search_query_parallel(q): return await asyncio.gather(*[_safe(p,q) for p in providers()])
def merge(items):
...
async def run_search(db,search):
    ...
    async def one(q):
        async with sem: return await search_query_parallel(q)
    ...
    for fp,best,xs,med,consensus,live_count in merge(items):
        ...
        if state=="BUY" and (previous is None or previous.state!="BUY"):
    ...
    run.provider_count=len(pnames); run.result_count=len(items); run.status="ok" if items else "empty"; run.finished_at=datetime.now(timezone.utc); run.details={"provider_errors":{k:v[:3] for k,v in errs.items()},"combinations":len(qs)}; db.commit(); return run
